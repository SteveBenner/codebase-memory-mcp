 ...
 * `origin` is the Origin header value and `if_none_match` the If-None-Match
 * header value ("" when absent) — the only two headers the routing layer
 * consumes. `body` is heap-allocated, NUL-terminated. */
typedef struct {
    char method[16];
    char path[2048];
    char query[2048];
